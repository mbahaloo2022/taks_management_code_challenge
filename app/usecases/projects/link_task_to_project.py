...
type LinkTaskToProjectOutput = Task


@dataclass(slots=True)
class LinkTaskToProjectInput:
    project_id: UUID
    task_id: UUID
...
class LinkTaskToProjectUseCase:
    ...
    def execute(self, input_: LinkTaskToProjectInput) -> LinkTaskToProjectOutput:
        ...
        now = utc_now()
        linked = task.assign_to_project(
            project_id=input_.project_id,
            project_deadline=project.deadline,
            now=now,
        )
        return self._tasks.save(linked)
